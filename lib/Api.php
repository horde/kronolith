...
class Kronolith_Api extends Horde_Registry_Api
{
                ...
                foreach ($iCal->getComponents() as $content) {
                    ...
                    if (!$complete_list) {
                        ...
                        try {
                            ...
                            if (isset($uid)) {
                                try {
                                    $existing = $kronolith_driver->getByUID($uid, [$calendar]);
                                } catch (Horde_Exception_NotFound $e) {
                                }
                            }
                        }
    ...
    public function listBy($action, $timestamp, $calendar = null, $end = null, $isModSeq = false)
    {
        if (empty($calendar)) {
            ...
            foreach ($cs as $c) {
                $results = array_merge(
                    $results,
                    $this->listBy($action, $timestamp, $c, $end, $isModSeq)
                );
            }
            return $results;
        }
    ...
    public function getChanges(
        ...
        foreach ($cs as $c) {
            ...
            if ($ignoreExceptions) {
                foreach ($uids as $uid) {
                    try {
                        $event = Kronolith::getDriver()->getByUID($uid, [$c]);
                    } catch (Exception $e) {
                        continue;
                    }
    ...
    protected function _addiCalEvent($content, $driver, $exception = false)
    {
        ...
        if (!$exception) {
            try {
                $driver->getByUID($event->uid, [$driver->calendar]);
     ...
     */
    public function export($uid, $contentType, array $options = [], ?array $calendars = null)
    {
    ...
    public function listEvents(
        ...
        if (!isset($calendars)) {
            $calendars = [$GLOBALS['prefs']->getValue('default_share')];
        } elseif (!is_array($calendars)) {
            $calendars = [$calendars];
        }
        foreach ($calendars as &$calendar) {
