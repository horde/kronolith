...
class KronolithUpgradeAddGeo extends Horde_Db_Migration_Base
{
    ...
    public function up()
    {
        ...
        if (!in_array('kronolith_events_geo', $tableList)) {
            $t = $this->createTable('kronolith_events_geo', ['autoincrementKey' => false]);
            $t->column('event_id', 'string', ['limit' => 32, 'null' => false]);
            $t->column('event_lat', 'string', ['limit' => 32, 'null' => false]);
            $t->column('event_lon', 'string', ['limit' => 32, 'null' => false]);
